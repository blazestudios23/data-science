...
```{r mean-samp1}
samp1 %>%
  summarise(x_bar = mean(area), sample_med = median(area), 
            standDev = sd(area), sample_iqr = IQR(area),
...
sample_means_small <- ames %>%
          ...
          summarise(x_bar = mean(area))
ggplot(data = sample_means_small, aes(x = x_bar)) + geom_histogram(binwidth = 40)
```
...
sample_meansSmall <- ames %>%
                    rep_sample_n(size = 15, reps = 2000, replace = TRUE) %>%
                    summarise(x_bar = mean(price))

ggplot(data = sample_meansSmall, aes(x = x_bar)) +
  geom_histogram(binwidth = 20)
